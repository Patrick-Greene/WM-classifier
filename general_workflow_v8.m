% general script to run any dataset (EFRI or LA or whatever)
% Using new layout of data. Uses new smoothing prior. Uses new channel
% distance from top of shank feature. Integrates over parameters beta
% (prior smoothing param) and alpha (kernel width in each dimension) via
% importance sampling (we use sampling rather than direct integration
% because we don't want to compute the normalizing constant p(D).

clear; clc

feature_type = "bp_dist"; %"bp_dist" "bipolar", %"common", "all" - indicates which features to use
% bp_dist = bipolar ref power feature and channel distance from top of shank 

src_dir = "/home/pg/my_programs/sarma_lab/bids_layout_data/wm_gm_features2/";
n_subj = 29; % total number of subjects
max_ch = 16; % max number of channels on any shank
n_samples = 100; % number of samples to use in posterior predictive (100 samples = 150 sec/patient)
alpha_cd = 1;

% Load all data
all_subj_data = {};
for subj_i=1:n_subj
    all_subj_data{subj_i} = load(src_dir + "subj_" + string(subj_i) + ".mat");
end

% order channels and add the new distance from top of shank feature
all_subj_data = order_channels(all_subj_data);
all_subj_data = add_feats(all_subj_data);


all_subj_est_probs = {}; % all_subj_est_probs{subj_i}{shank_i} is the marginal posterior prob that each contact
% is a wm contact for subj_i
for excl_subj_i = 1:n_subj % subject indx to exclude
...
[features_wm, features_gm] = make_feature_arrays(all_subj_data, excl_subj_i);

if feature_type == "bipolar"
    features_wm = features_wm(:,1);
    features_gm = features_gm(:,1);
elseif feature_type == "common"
    features_wm = features_wm(:,2);
    features_gm = features_gm(:,2);
elseif feature_type == "bp_dist"
    features_wm = features_wm(:,[1 3]);
    features_gm = features_gm(:,[1 3]);
    ch_dist_indx = 2; % second component of feature array is ch_dist
elseif feature_type == "all"
    ch_dist_indx = 3; % third component of feature array is ch_dist  
end
...
for subj_i=1:n_subj
    ...
    for shank_i=1:n_shanks
        train_labels{end+1} = all_subj_data{subj_i}.subj_data.ch_type_list{shank_i};
    end
end
...
test_features = {};
test_labels = {};
n_shanks = length(all_subj_data{excl_subj_i}.subj_data.ch_type_list);
for shank_i=1:n_shanks
    if feature_type == "bipolar"
        test_features{shank_i} = all_subj_data{excl_subj_i}.subj_data.ch_feat_list{shank_i}(:,1);
    elseif feature_type == "common"
        test_features{shank_i} = all_subj_data{excl_subj_i}.subj_data.ch_feat_list{shank_i}(:,2);
    elseif feature_type == "bp_dist"
        test_features{shank_i} = all_subj_data{excl_subj_i}.subj_data.ch_feat_list{shank_i}(:,[1 3]);    
    elseif feature_type == "all"
        test_features{shank_i} = all_subj_data{excl_subj_i}.subj_data.ch_feat_list{shank_i};
    end
    
    test_labels{shank_i} = all_subj_data{excl_subj_i}.subj_data.ch_type_list{shank_i}';
end
...
all_subj_est_probs{excl_subj_i} = marg_posterior_predictive(features_wm, features_gm,...
    train_labels, test_features, ch_dist_indx, max_ch, @prior_pdf5,...
    alpha_cd, n_samples);

end

% save("/home/pg/my_programs/sarma_lab/classifier/output_results/all_subj_est_probs_sampled_"...
%     +feature_type, 'all_subj_est_probs')

%%
% loglik = calc_alpha_loglik(features_wm, features_gm, 2, 0.5, 0.9) % about 4 sec per eval
...
% log_un_posterior_params(1, 1, 5, features_wm, features_gm, labels, 16, 2)
% gauss_approx_posterior_params_sampler(5, rand(1,3), eye(3))
...
% [x,fval,exitflag,output,lambda,grad,hessian] = fmincon(@(x) -log_un_posterior_params(...
%     x(1:n_feat), x(n_feat+1:2*n_feat), x(2*n_feat+1), features_wm,features_gm,...
%     labels, max_ch, ch_dist_indx), [alpha_wm0 alpha_gm0 beta0],...
%     -eye(2*n_feat+1), zeros(2*n_feat+1,1));
% 
% 
% toc
...
%     x, labels, max_ch), [beta0], -eye(1), zeros(1));
% 
% toc
...
% alpha_cd = 1;
% n_feat = length(alpha_wm0);
% options = optimoptions('fmincon','StepTolerance',0.001);
...
% tic
% % fmindbnd takes 30% longer than 1D fmincon, is less accurate,
% and doesn't return the hessian
% alpha_gm0 = 0.2;
% alpha_cd = 1;
% n_feat = length(alpha_gm0);
% options = optimoptions('fmincon','StepTolerance',0.001);
% x = fminbnd(@(x) -log_un_posterior_alpha_generic(...
%     x,features_gm, ch_dist_indx, alpha_cd), 0, 1);
% 
% toc
...
[mu, sigma] = calc_max_params(alpha_wm0, alpha_gm0, beta0,...
    features_wm,features_gm, train_labels, max_ch, ch_dist_indx, alpha_cd);
toc

% 0.157106560920128   0.137451162226986
% True Hessian (using hessianest):
% 1.0e+03 *
% 
%    4.331026998357396   0.000000000409440
%    0.000000000409440   7.313046696038329

% err =
% 
%    1.0e+02 *
% 
%    0.000000276887726   0.000000037152887
%    0.000000037152887   3.685259072566930

% x = [0.1:0.005:0.2];
% vals = [];
% for i=1:length(x)
%     vals(i) = -log_un_posterior_alpha(...
%      0.157, x(i), features_wm,features_gm, ch_dist_indx);
% end
% plot(x,vals)

% [hess,err] = hessianest(@(x) -log_un_posterior_alpha(...
%     x(1:n_feat), x(n_feat+1:end), features_wm,features_gm, ch_dist_indx),...
%     [0.1571065, 0.137451])

% tic
% n_samples = 100; % number of samples to use in posterior predictive
% alpha_cd = 1;
% est_probs = marg_posterior_predictive(features_wm, features_gm,...
%     train_labels, test_features, ch_dist_indx, max_ch, @prior_pdf5, alpha_cd, n_samples);
% toc

% n_samples = 2;
% samples = [0.1571, 0.13745, 7.5659; 0.1571, 0.13745, 7.5659];
% sample_norm_const = {};
...
% norm_const = calc_normalizing_constants(max_ch,7.5659);
% marg_shank_posterior(features_wm, features_gm,...
%     test_features{1}, ch_dist_indx, @prior_pdf5, 1, 0.1, 0.1, 7.5659, norm_const)

% [est_probs, MAP_est, ML_est] = wm_bayes_classifier(features_wm, features_gm,...
%     test_features, ch_dist_indx, @(x) prior_pdf5(x, 7.5659, norm_const));
...
%     marg_prob_samples = [];
%     for sample_i=1:n_samples
%         marg_prob_samples(end+1,:) = marg_shank_posterior(features_wm, features_gm,...
...
function est_probs = marg_posterior_predictive(train_features_wm, train_features_gm,...
    train_labels, test_features, ch_dist_indx, max_ch, prior_pdf, alpha_cd, n_samples)
% This function replaces wm_bayes_classifier. It's the same except it
% integrates over the posterior distribution of the parameters
...
for shank_i=1:n_shanks
    ...
    for sample_i=1:n_samples
        marg_prob_samples(end+1,:) = marg_shank_posterior(train_features_wm, train_features_gm,...
            test_features{shank_i}, ch_dist_indx, prior_pdf, alpha_cd,...
            samples(sample_i,1:n_feat), samples(sample_i,n_feat+1:2*n_feat), samples(sample_i,end),...
            sample_norm_const{sample_i});
    end
    est_probs{shank_i} = mean(marg_prob_samples,1);
...
function out = order_channels(all_subj_data)
% Turns out the channels aren't always in numerical (or reversed) order w/in a shank.
% While we're at it, we can put the la channels in the same order as the
% EFRI channels (i.e. A10 A9 ... A1)
n_subj = length(all_subj_data);
for subj_i=1:n_subj
    n_shanks = length(all_subj_data{subj_i}.subj_data.ch_feat_list);
    for shank_i=1:n_shanks
        n_ch = length(all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i}); 
        ch_name_list = all_subj_data{subj_i}.subj_data.ch_name_list{shank_i}; % cell array
        ch_type_list = all_subj_data{subj_i}.subj_data.ch_type_list{shank_i};
        ch_feat_list = all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i};
        
        ch_nbr_list = []; % the ch number; i.e. A10 -> 10
        for ch_i=1:n_ch
            ch_nbr_list(end+1) = str2double(regexp(ch_name_list{ch_i},'\d*','Match'));
        end
        [~,indices] = sort(ch_nbr_list,'descend');
        all_subj_data{subj_i}.subj_data.ch_name_list{shank_i} = ch_name_list(indices);
        all_subj_data{subj_i}.subj_data.ch_type_list{shank_i} = ch_type_list(indices);
        all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i} = ch_feat_list(indices,:);
    end
end
out = all_subj_data;
end

function out = add_feats(all_subj_data)
% add some additional features to all_subj_data. Returns all_subj_data with
% new feature columns attached:
% all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i} is n_ch x n_feat array

% new feat 1: distance (n_ch away) from first good channel on shank
n_subj = length(all_subj_data);
for subj_i=1:n_subj
    n_shanks = length(all_subj_data{subj_i}.subj_data.ch_feat_list);
    for shank_i=1:n_shanks
        ...
        % channel indx closest to skull; for both LA and EFRI this is the
        % largest ch number (i.e. A10), but for EFRI this is the first ch index while for
        % LA this is the last ch index. However, I've now reordered all
        % channels to be in the EFRI ordering.
        outermost_indx = str2double(regexp(ch_name_list{1},'\d*','Match'));
            
        ch_dist = [];
        for ch_i=1:n_ch
            indx = str2double(regexp(ch_name_list{ch_i},'\d*','Match'));
            ch_dist(end+1,1) = outermost_indx - indx;
        end
        all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i} = ...
            [all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i}, ch_dist];
    end
end
out = all_subj_data;
end
...
function [est_probs, MAP_est, ML_est] = wm_bayes_classifier(train_features_wm, train_features_gm,...
    test_features, ch_dist_indx, prior_pdf)
...
est_probs = {}; % est_probs{shank_i} is a vector of marginal probabilities that each electrode
% is white matter
MAP_est = {}; % MAP_est{shank_i} is a binary vector giving the most likely posterior labeling
ML_est = {}; % ML_est{shank_i} is a binary vector giving the max likelihood labeling
% gm_lik{shank_i} is likelihood for all elecs on shank i, under the gm
% distribution. Similarly for wm_lik but under wm distribution
gm_lik = {};
wm_lik = {};
n_shanks = length(test_features);
for shank_i=1:n_shanks
    if length(test_features{shank_i}) > 0 % skip shanks w/ no electrodes
        %[gm_lik{shank_i},~] = ksdensity(train_features_gm,test_features{shank_i});
        %[wm_lik{shank_i},~] = ksdensity(train_features_wm,test_features{shank_i});
        gm_lik{shank_i} = calc_likelihood(train_features_gm, test_features{shank_i}, ch_dist_indx);
        wm_lik{shank_i} = calc_likelihood(train_features_wm, test_features{shank_i}, ch_dist_indx);
        
        % get the max likelihood estimate
        ML_est{shank_i} = (wm_lik{shank_i} > gm_lik{shank_i})';


        % generate all possible wm/gm labelings of the electrodes on shank_i,
        % and the associated likelihood*prior (un_post) for each
        [labelings, un_post] = gen_labelings(gm_lik{shank_i}, wm_lik{shank_i}, prior_pdf);
        normalizing_const = sum(un_post); % bayes normalizing constant
        
        % get the MAP estimate
        [max_val, max_indx] = max(un_post);
        MAP_est{shank_i} = labelings(max_indx,:);

        % get the marginal probability estimate
        n_elecs = length(gm_lik{shank_i});
        marg_probs = nan(1,n_elecs);
        for elec_i=1:n_elecs
            % Note labelings(:,elec_i) gives all the rows where elec_i == 1
            marg_probs(elec_i) = sum(un_post(logical(labelings(:,elec_i))));
        end
        est_probs{shank_i} = marg_probs/normalizing_const;
    end
...
function lik = calc_likelihood(train_features, test_features, ch_dist_indx)
% train_features: N x n_features array, giving features for all wm or gm training contacts
% test_features: n_ch x n_features feature array, giving features for each
% contact on a particular shank
% ch_dist_indx: the index in the feature vector of the component representing distance from top of shank
...
% original version
% [lik,~] = ksdensity(train_features,test_features);

% New version
% Since distance from top of shank is really an integer-valued variable, need to
% integrate from distance-0.5 to distance+0.5

% compute bandwidth in each dimension using silverman's rule of thumb
[n_data, n_feat] = size(train_features);
bandwidth = 1.0592*std(train_features)*(n_data^(-1/5));
...
% A fn that generates all possible labelings of the shank contacts,
% and the associated likelihood*prior. Then the sum of all the entries
% where contact j=1 divided by the sum of all the entries is the marginal
% posterior probability that j=1. 
function [labelings, u_post] = gen_labelings(shank_gm_lik, shank_wm_lik, prior_pdf)
...
u_post = nan(2^n_elecs,1);
for i=1:2^n_elecs
    lik = prod(shank_wm_lik(logical(labelings(i,:)))) * ...
        prod(shank_gm_lik(~logical(labelings(i,:))));
    
    prior = prior_pdf(labelings(i,:));
...
function [features_wm, features_gm] = make_feature_arrays(all_subj_data, excl_subj_i)
...
for subj_i=1:n_subj
    ...
    n_shanks = length(all_subj_data{subj_i}.subj_data.ch_type_list);
    for shank_i=1:n_shanks
        wm_mask = logical(all_subj_data{subj_i}.subj_data.ch_type_list{shank_i});
        gm_mask = ~wm_mask;
        features_wm = [features_wm; all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i}(wm_mask,:)];
        features_gm = [features_gm; all_subj_data{subj_i}.subj_data.ch_feat_list{shank_i}(gm_mask,:)];
    end
end
